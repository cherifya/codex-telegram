"""Tests for the Codex-backed Claude compatibility integration layer."""
...
def manager(config: Settings) -> ClaudeSDKManager:
    with patch(
        "src.claude.sdk_integration.find_codex_cli", return_value="/usr/bin/codex"
    ):
        return ClaudeSDKManager(config)


class TestCodexBackedClaudeSDKManager:
    async def test_execute_command_success(self, manager: ClaudeSDKManager):
        ...
        assert "--output-last-message" in called_cmd
    ...
    async def test_execute_command_disable_yolo_uses_workspace_sandbox(
    ...
    async def test_execute_command_stream_callback(self, manager: ClaudeSDKManager):
        ...
        assert any(
            update.type == "stream_delta" and update.content == "partial"
    ...
    async def test_extract_completed_response_text(self, manager: ClaudeSDKManager):
        async def _create_process(*cmd, **kwargs):
            payload = {
